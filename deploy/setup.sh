...
apt-get install -y -qq \
  ...
  >/dev/null
# Node 20+ is installed and version-checked by the shared runtime ensure step
# below for the Claude CLI. The same step installs the pinned agent-browser
# binary used by the apply runtime.
