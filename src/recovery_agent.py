"""Autonomous diagnosis and self-repair for failed apply attempts.
...
RECOVERY_ENABLED = os.environ.get("RECOVERY_ENABLED", "1") != "0"
RECOVERY_MODEL = os.environ.get("RECOVERY_MODEL", os.environ.get("APPLY_MODEL", "deepseek-v4-pro"))
RECOVERY_MAX_TURNS = int(os.environ.get("RECOVERY_MAX_TURNS", "18"))
RECOVERY_MAX_TOKENS = int(os.environ.get("RECOVERY_MAX_TOKENS", "12000"))
RECOVERY_TIMEOUT_SECONDS = int(os.environ.get("RECOVERY_TIMEOUT_SECONDS", "900"))
RECOVERY_VERIFY_CMD = os.environ.get("RECOVERY_VERIFY_CMD", "just check")
RECOVERY_DEPLOY_CMD = os.environ.get("RECOVERY_DEPLOY_CMD", "just deploy")
RECOVERY_ALLOW_CODE_CHANGES = os.environ.get("RECOVERY_ALLOW_CODE_CHANGES", "1") != "0"
RECOVERY_ALLOW_DEPLOY = os.environ.get("RECOVERY_ALLOW_DEPLOY", "1") != "0"
RECOVERY_ALLOW_DIRTY_WORKTREE = os.environ.get("RECOVERY_ALLOW_DIRTY_WORKTREE", "0") == "1"
RECOVERY_REQUIRE_MAIN = os.environ.get("RECOVERY_REQUIRE_MAIN", "1") != "0"

DEFAULT_RECOVERY_OUTCOMES = {
    "blocked",
    "error",
    "incomplete",
    "login_required",
...
RECOVERY_OUTCOMES = {
    ...
    for s in os.environ.get(
        "RECOVERY_OUTCOMES",
        ",".join(sorted(DEFAULT_RECOVERY_OUTCOMES)),
    ).split(",")
    if s.strip()
}

RUN_LOG = LOG_DIR / "recovery.jsonl"
...
def should_recover(status: str | None) -> bool:
    return RECOVERY_ENABLED and (status or "") in RECOVERY_OUTCOMES


def recover_after_apply(
    *,
    listing: dict,
    result: AgentResult,
    trigger: str,
    ...
    """Run recovery for a failed apply result when configured to do so.

    Never raises into the caller; the application pipeline must continue even if
    self-recovery fails.
    """
    if not should_recover(result.outcome):
        return None
    ctx = {
        "listing": listing,
        "result": {
    ...
    try:
        ...
        _log("done", status=result.outcome, action=rr.action,
             ...
             summary=rr.summary)
        return rr
    except Exception as e:  # noqa: BLE001 - recovery must be best-effort
        _log("error", status=result.outcome, error=f"{type(e).__name__}: {e}")
        try:
            send_alert(
                "⚠️ Recovery agent failed",
                f"The recovery agent crashed while handling {result.outcome}.\n\n"
                f"{type(e).__name__}: {e}\n\n"
                f"Listing: {listing.get('source_url') or '-'}",
            )
        except Exception:
            pass
        return RecoveryResult(action="error", summary=f"{type(e).__name__}: {e}")
...
def run_recovery(context: dict) -> RecoveryResult:
    log_path = _new_log_path()
    logger = _Logger(log_path)
    try:
        logger.line(f"[recovery] model={RECOVERY_MODEL} status={context['result']['outcome']}")
        ...
        return RecoveryResult(action="timeout", summary="Recovery agent timed out.")
    finally:
        logger.close()


async def _run(context: dict, logger: "_Logger") -> RecoveryResult:
    api_key = os.environ.get("DEEPSEEK_API_KEY")
    if not api_key:
        logger.line("[recovery] DEEPSEEK_API_KEY not set; emailing user")
        send_alert(
            "⚠️ Rental recovery needs configuration",
            "The recovery agent could not run because DEEPSEEK_API_KEY is not set.",
        )
    ...
    for turn in range(1, RECOVERY_MAX_TURNS + 1):
        ...
        logger.line(f"[recovery] turn {turn} finish={choice.finish_reason} calls={len(calls)}")
        messages.append({
            "role": "assistant",
        ...
        if msg.content:
            logger.line(f"[recovery] say: {msg.content.strip()[:500]}")
        if not calls:
            ...
            messages.append({
                "role": "user",
                "content": "Return only the required RECOVERY_JSON object, or call another tool.",
            })
            continue
        ...
        for tc in calls:
            try:
                ...
                args = {}
            logger.line(f"[recovery] call {tc.function.name} {_safe_args(args)}")
    ...
    return RecoveryResult(action="incomplete", summary="Hit recovery turn budget.")
...
- Browser clicks are diagnostic only. Use browser_safe_click only for benign
  ...
  or change account settings from recovery.
...
RECOVERY_JSON: {{"action":"noop|emailed_user|fixed_deployed|fix_failed|error","root_cause":"...","summary":"...","email_sent":false,"code_changed":false,"deployed":false}}
"""
...
async def _call_tool(name: str, args: dict, context: dict, logger: "_Logger") -> str:
    try:
        ...
        if name == "run_verification":
            return _run_shell(RECOVERY_VERIFY_CMD, timeout=300)
        if name == "git_status":
            return _git_status()
        if name == "git_diff":
            return _run(["git", "diff"], timeout=30)
        if name == "commit_push_deploy":
            return _commit_push_deploy(args.get("message", "fix(recovery): repair apply failure"))
        if name == "send_user_email":
            send_alert(str(args.get("subject") or "Rental agent needs attention"),
                       redact(str(args.get("body") or "")))
            return "email sent"
    except Exception as e:  # noqa: BLE001
        logger.line(f"[recovery] tool error: {type(e).__name__}: {e}")
...
async def _browser_screenshot_async(full_page: bool) -> str:
    ...
    async with async_playwright() as p:
        ...
        try:
            page = await _current_page(browser)
            path = SCREENSHOT_DIR / f"recovery_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
...
def _apply_diff(diff: str) -> str:
    if not RECOVERY_ALLOW_CODE_CHANGES:
        return "REFUSED: RECOVERY_ALLOW_CODE_CHANGES=0"
    if not RECOVERY_ALLOW_DIRTY_WORKTREE and _porcelain():
        return "REFUSED: worktree is dirty before recovery patch"
    if not diff.strip():
        return "empty diff"
...
def _commit_push_deploy(message: str) -> str:
    if not RECOVERY_ALLOW_CODE_CHANGES:
        return "REFUSED: RECOVERY_ALLOW_CODE_CHANGES=0"
    branch = _run(["git", "rev-parse", "--abbrev-ref", "HEAD"], timeout=10).strip()
    if RECOVERY_REQUIRE_MAIN and branch not in {"main", "master"}:
        return f"REFUSED: current branch is {branch!r}, not main/master"
    if not _porcelain():
        return "nothing to commit"
    verify = _run_shell(RECOVERY_VERIFY_CMD, timeout=300)
    ...
    if not push.startswith("rc=0\n"):
        return "push failed\n" + push
    if not RECOVERY_ALLOW_DEPLOY:
        return "committed and pushed; deploy skipped because RECOVERY_ALLOW_DEPLOY=0"
    deploy = _run_shell(RECOVERY_DEPLOY_CMD, timeout=600)
    return "committed, pushed, deploy attempted\n" + deploy
...
def _validate_diff_paths(diff: str) -> None:
    ...
    for raw in paths:
        ...
        if not raw.startswith(allowed_prefixes):
            raise ValueError(f"diff path not allowed for recovery: {raw}")
...
def _parse_final(text: str) -> RecoveryResult | None:
    m = re.search(r"RECOVERY_JSON:\s*(\{.*\})", text or "", re.DOTALL)
    if not m:
        return None
    try:
...
def _commit_message(message: str) -> str:
    ...
    if not re.match(r"^(fix|chore|test|docs|refactor)(\([^)]+\))?: ", first):
        return "fix(recovery): repair failed application flow"
    return first


def _new_log_path() -> Path:
    path = LOG_DIR / "recovery" / f"{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    path.parent.mkdir(parents=True, exist_ok=True)
    return path
...
def _log(event: str, **kw) -> None:
    ...
    print(f"[recovery] {event}: " + " ".join(f"{k}={v}" for k, v in rec.items() if k != "event"))
