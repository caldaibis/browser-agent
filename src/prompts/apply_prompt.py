...
def build_prompt(listing: Listing | dict) -> str:
    ...
    login_clause = (
        ...
        "first check whether the listing page provides a contact email address "
        "(mailto link or email text like \"info@domain.nl\") or a \"Neem contact op\" "
        "link. If a public email contact exists, use that channel: compose the "
        "application message addressed to that email and report the email channel "
        "as the application method. Only stop and report login_required if there "
        "is genuinely no email contact alternative accessible without login. "
        "NEVER reset, change, or recover a "
